...
/* ───── animated bar chart ───── */
function BarChart({ data, color, gradientFrom, gradientTo }: {
      ...
      {data.map((item, i) => {
        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center gap-1.5 group">
            {/* value tooltip */}
            <span className="text-[10px] font-bold text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-white shadow-md rounded-md px-1.5 py-0.5 border border-slate-100">
              {item.value}
            </span>
            {/* bar */}
            ...
            {/* label */}
            <span className="text-[8px] sm:text-[9px] text-slate-400 truncate w-full text-center leading-none">
              {item.label}
            </span>
          </div>
        );
...
export default function Dashboard() {
  ...
  const pageLabels: Record<string, string> = {
    '/': 'Home', '/#home': 'Home', '/#services': 'Services', '/#about': 'About', '/#contact': 'Contact',
  };
          ...
          <div className="flex items-center justify-between h-16">
            <a href="/" className="flex items-center gap-3 group">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-linear-to-r from-blue-500 to-indigo-500 text-white shadow-sm">
                  LIVE
                </span>
              </div>
            </a>
            ...
            <div className="flex items-center gap-3">
              ...
              <a href="/"
                className="text-xs font-medium text-slate-500 px-3 py-1.5 rounded-full border border-slate-200 hover:bg-slate-50 hover:border-slate-300 transition-all duration-200">
                ← Back to Site
              </a>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5 mb-8">
          {statsCards.map((card, idx) => (
            <div key={card.label}
              ...
              <div className="text-2xl lg:text-3xl font-extrabold text-slate-900 tracking-tight mb-0.5">
                      ...
                      <div className="flex items-center justify-between text-sm mb-1.5">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="font-medium text-slate-700 truncate text-xs">
                            {pageLabels[page.page] || page.page || 'Home'}
                          </span>
                        </div>
                        <span className="text-xs font-bold text-slate-900 bg-slate-100 px-2 py-0.5 rounded-full ml-2 shrink-0">
                          {page.count}
                        </span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                        <div
                          className={`h-full rounded-full bg-linear-to-r ${colors[idx % colors.length]} transition-all duration-700 ease-out`}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {[...dailyStats].reverse().map((day, idx) => (
                      <tr key={day.date}
                        className={`border-b border-slate-50/80 hover:bg-slate-50/50 transition-colors ${idx === 0 ? 'bg-blue-50/30' : ''}`}>
                        ...
                        <td className="py-2.5 text-right">
                          <span className="inline-flex items-center justify-center min-w-8 px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-xs font-bold">
                            {day.visitors}
                          </span>
                        </td>
                        <td className="py-2.5 text-right">
                          <span className="inline-flex items-center justify-center min-w-8 px-2 py-0.5 rounded-full bg-orange-50 text-orange-700 text-xs font-bold">
                            {day.pageviews}
                          </span>
                        </td>
                        <td className="py-2.5 text-right text-xs text-slate-500 hidden sm:table-cell font-medium">
