...
export function trackVisitorLocally(page: string) {
  try {
    const visits: Array<{ page_path: string; referrer: string; user_agent: string; visited_at: string }> = JSON.parse(localStorage.getItem("fainaya_visits") || "[]");
    visits.push({
      page_path: page,
...
export function getLocalTotalStats() {
  ...
  return {
    ...
    avgVisitorsPerDay: daily.length > 0 ? Math.round(daily.reduce((sum, d) => sum + d.visitors, 0) / daily.length) : 0,
...
export async function fetchSupabaseDailyStats(days: number = 14): Promise<DailyStats[]> {
  try {
    ...
    const { data, error } = await supabase.from("visitor_logs").select("visited_at, page_path").gte("visited_at", cutoffStr).order("visited_at", { ascending: true });
    ...
    for (const row of data) {
      ...
      const fp = row.visited_at?.slice(0, 16) + row.page_path; // minute-level uniqueness
      day.visitors.add(fp);
    }
...
export async function fetchSupabasePageStats(days: number = 30): Promise<PageStats[]> {
  try {
    ...
    const { data, error } = await supabase.from("visitor_logs").select("page_path").gte("visited_at", cutoffStr);
